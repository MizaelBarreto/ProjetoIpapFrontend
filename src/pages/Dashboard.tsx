...
const Dashboard: React.FC = () => {
 ...
 const handleExport = () => {
  ...
  respostas.forEach((r) => {
    ...
    const keysToExport =
      selecionadas.length === 0 || selecionadas.includes("total") ? Object.keys(respostasObj) : selecionadas;
    const keys = Array.isArray(keysToExport) ? keysToExport : Object.keys(respostasObj);

    // prepara string de detalhes (categorias) por respondente
    const categories = (r.resultado && r.resultado.categories) ? r.resultado.categories : {};
    const detailsStr = Object.entries(categories)
      .map(([k, v]) => `${k}: ${v}`)
      .join(" | ");

    const finalCategory = r.resultado?.finalCategory ?? "Indefinido";

    // gera uma linha por pergunta/valor selecionado
    keys.forEach((k) => {
      const respVal = respostasObj[k] ?? "";
      csv += `"${safe(r.nome)}","${safe(r.email)}","${safe(perguntasMap[k] ?? k)}","${safe(
        respVal
      )}","${safe(finalCategory)}","${safe(detailsStr)}"\n`;
    });
  });
